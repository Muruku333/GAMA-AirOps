...
export default function EditAircraft(props) {
  const { handleTabChange, idToEdit, setStatus, loggedUser } = props;
  ...
  useEffect(() => {
    ...
    const fetchData = async() =>{
      try {
        await axios.get(`${API_URL}/api/aircrafts/${idToEdit}`).then((response)=>{
          if(response.data.status){
            const { reg_no, min_cabin_crew, min_flight_crew, no_of_captain, no_of_fo, no_of_fe, c_cls_capacity, y_cls_capacity, seating_capacity, time_format, local_time, utc_time, block_opening_hrs, time_in_air_opening_hrs, not_in_service, not_in_service_from, freight_capacity, unit, operator, aircraft_model} = response.data.results[0];
            setAircraftData({
              operatorId: operator.operator_id,
              regNo: reg_no,
              modelId: aircraft_model.model_id,
              minCabinCrew: min_cabin_crew,
              minFlightCrew: min_flight_crew,
              noOfCaptain: no_of_captain,
              noOfFo: no_of_fo,
              noOfFe: no_of_fe,
              cClsCapacity: c_cls_capacity,
              yClsCapacity: y_cls_capacity,
  ...
  const validateAircraftData = () => {
    ...
    if (!Boolean(aircraftData.regNo)) errors.regNo = "Reg. No is required.";
    if (!Boolean(aircraftData.modelId)) errors.modelId = "Model is required.";
  ...
  const handleSubmitAircraft = async (event) => {
    ...
    try {
      await axios
        .put(`${API_URL}/api/aircrafts/${idToEdit}`, {
          ...aircraftData,
          modifiedBy: loggedUser.user_id,
          notInServiceFrom: fDate(aircraftData.notInServiceFrom,"yyyy-MM-dd"),
        })
        .then((response) => {
            ...
            >
              <Grid item xs={4} sm={6} md={6}>
                <Autocomplete
                  ...
                  value={
                    operators.find(
                      (_op) => _op.operator_id === aircraftData.operatorId
                    ) || null
                  }
              ...
              <Grid item xs={4} sm={6} md={4}>
                ...
                <Box display={"flex"} justifyContent={"center"}>
                  <TextField
                    ...
                    onChange={(event) => {
                      handleInputChange("freightCapacity", event.target.value);
                    }}
                  />
                  <Autocomplete
                    size="small"

...
export default function EditCTorDM(props) {
  ...
  useEffect( ()=>{
    ...
    const fetchData = async() =>{
      try {
        await axios.get(`${API_URL}/api/crew_training_document_master/${idToEdit}`).then((response)=>{
          if(response.data.status){
            const {type, ctdm_name, group_code, warning_days, frequency_unit, frequency, renewal_period} = response.data.results[0];
            setCDTMData({
              type,
              ctdmName:ctdm_name,
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await axios.put(`${API_URL}/api/crew_training_document_master/${idToEdit}`,{...CTDMData,modifiedBy:loggedUser.user_id}).then((response)=>{
  ...
  return (
    <>
      ...
      >
        <DialogTitle>{"Edit Crew Training/Document Master"}</DialogTitle>
        <DialogContent>

...
// import Hawker900XP from "./Images/Hawker900XP.png";
...
export default function CreateQuotation({
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      await axios
        .get(`${DB_URL}/api/quotations`)
        .then((data) => setCustomersRows(data.data))
        .catch((error) => console.log(error));
      ...
      await axios
        ...
        .then((data) => {
          ...
          const quotation_no = `QU-${today.getFullYear()}/SAPL/VT-VPA/${id
            .toString()
            .padStart(4, "0")}`;
      ...
      await axios
        .get(`${DB_URL}/api/user/${loggedInUserId}`)
        .then((data) => {
  ...
  const handleSubmit = async (catType) => {
    ...
    try {
      ...
      setTimeout(async () => {
        const chargesData = { customer_id: cus_id, chargeBoxes: chargeBoxes };
        await axios
          .post(`${DB_URL}/api/charge_boxes`, chargesData)
          .then((res) => {
      ...
      setTimeout(async () => {
        await axios
          ...
          .then(async (response) => {
            ...
            // const quotation_no = "SAPL/QU" + today.getFullYear() + "/";
            const quotation = {
            ...
            await axios
              .post(`${DB_URL}/api/quotations`, quotation)
              .then((response) => {
                axios
                  .get(`${DB_URL}/api/quotations`)
                  .then((data) => getPDF(cus_id))
                  .catch((error) => console.log(error));
              });
          });
      }, 500);
    ...
    try {
      setTimeout(async () => {
        ...
        if (tripDetails.length <= 6 && tripDetails.length !== 1) {
          ...
          data =
            await require(`./PDFTemplates/${quotesDetails["label"]}_6.json`);
        }

        // let response;
        // if (quotesDetails["label"] === "Hawker 800XP") {
        //   response = await fetch(Hawker800XP);
        // }
        // if (quotesDetails["label"] === "Hawker 900XP") {
        //   response = await fetch(Hawker900XP);
        // }
        ...
        // reader.onloadend = () => {
        //   const specImgData = reader.result;
        //   // Use base64data in your PDF generation
        //   console.log(specImgData);
        ...
        chargeBoxes.map((charge, index) => {
          data_3 = {
            ...
            ...{
              ...
              [`filed_value_${index + 1}`]: `₹ ${formatCurrency(
  ...
  return (
    <>
      <Helmet>
        <title>Quotation Create | GAMA AirOps </title>
      </Helmet>
        ...
        >
          <Box>
            <Typography variant="h4" gutterBottom>Create&nbsp;a&nbsp;new&nbsp;quotation</Typography>
            <Breadcrumbs
                ...
                <Grid item xs={4} sm={6} md={6}>
                  <Grid container spacing={1} columns={{xs:4,sm:6,md:6}}>
                    ...
                    <LocalizationProvider dateAdapter={AdapterDayjs}>
                      <DemoContainer sx={{ mt: -1 }} components={["DateField"]}>
                        <DateField
                          disabled
                          fullWidth
                          size="small"
                    ...
                    <Tooltip title="Change Title" placement="right">
                      <IconButton
                        ...
                        onClick={(event) => {
                          ...
                          setTitleValue(chargeBoxes[4]["title"]);
                        }}
                      >

...
import ListHeadQuotation from "./ListHeadQuotation";
import ListToolbarQuotation from "./ListToolbarQuotation";
...
const TABLE_HEAD = [
  ...
  { id: "quotation_no", label: "Quotation No", alignRight: false },
  { id: "customer_name", label: "Customer Name", alignRight: false },
  { id: "label", label: "Aircraft Type", alignRight: false },
...
export default function ListQuotation({
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = customersRows.map((n) => n.quotation_no);
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - customersRows.length) : 0;
  ...
  return (
    <>
      <Helmet>
        <title> Quotation List | GAMA AirOps </title>
      </Helmet>
